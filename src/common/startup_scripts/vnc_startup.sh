...
function start_kasmvnc (){
    ...
    if [[ "${BUILD_ARCH}" =~ ^aarch64$ ]] && [[ -f /lib/aarch64-linux-gnu/libgcc_s.so.1 ]] ; then
		LD_PRELOAD=/lib/aarch64-linux-gnu/libgcc_s.so.1 vncserver $DISPLAY -rfbport $NO_VNC_PORT -depth $VNC_COL_DEPTH -geometry $VNC_RESOLUTION -httpd ${KASM_VNC_PATH}/www -interface 0.0.0.0
	else
		vncserver $DISPLAY -rfbport $NO_VNC_PORT -depth $VNC_COL_DEPTH -geometry $VNC_RESOLUTION -httpd ${KASM_VNC_PATH}/www -interface 0.0.0.0
	fi
...
function start_gamepad (){
	if [[ ${KASM_SVC_GAMEPAD:-1} == 1 ]]; then
		...
		$STARTUPDIR/gamepad/kasm_gamepad_server --auth-token "kasm_user:$VNC_PW" &
